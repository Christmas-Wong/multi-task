# -*- coding: utf-8 -*-
"""
@Time    : 2021/9/18 2:25 下午
@Author  : Fei Wang
@File    : model
@Software: VS Code
@Description: Single Encoder Multi-Head Model(https://user-images.githubusercontent.com/6007894/134903082-64247dd0-fc6f-4b85-a936-b9607ade5a14.png)
"""
import torch
...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def predict(
        ...
        pooled_output = self.dropout(pooled_output)
        result = {}
        for ele in list(self.num_labels.keys()):
            logits = None
            if ele == list(self.num_labels.keys())[0]:
                logits = self.classifier1(pooled_output)
            elif ele == list(self.num_labels.keys())[1]:
                logits = self.classifier2(pooled_output)
            if True:
                ...
                result[ele] = predictions_code[0].item()
        return result